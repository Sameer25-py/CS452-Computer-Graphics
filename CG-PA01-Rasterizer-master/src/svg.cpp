 ...
 */
 Color ColorTri::color(float x0, float y0,float x1, float y1,float x2, float y2, float px, float py, SampleParams sp) {
   ...
   //cout << "entered" << endl;
   float area_abc = 0.5 * (x0*y1 + x1*y2 + x2*y0 - x0*y2 - x1*y0 - x2*y1);
   float area_apc = 0.5 * (x0*py + px*y2 + x2*y0 - x0*y2 - px*y0 - x2*py);
   float area_abp = 0.5 * (x0*y1 + x1*py + px*y0 - x0*py - x1*y0 - px*y1);

   float beta = area_apc / area_abc;
   float gamma = area_abp / area_abc;
   float alpha = 1 - beta - gamma;

   Color wcolor = this->p0_col * alpha + this->p1_col * beta + this->p2_col * gamma;
//...
   //cout << "returned" << endl;
   return wcolor;
 }
 ...
 Color TexTri::color(float x0, float y0,float x1, float y1,float x2, float y2, float px, float py, SampleParams sp) {
   ...
   Vector2D result = this->p0_uv * alpha + this->p1_uv * beta + this-> p2_uv * gamma;
   sp.p_uv = result;
