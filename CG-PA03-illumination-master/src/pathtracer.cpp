...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  int numOf_samples = (scene->lights).size();
  Vector3D wi = (0, 0, 0);
  float distToLight=0.0, pdf=0.0;
  Vector3D w_in = (0, 0, 0);

  //cout << "samples: " << numOf_samples << endl;
  for(int i=0; i < numOf_samples; i++){
  	//if delta light
  	Spectrum singleSample;
  	Spectrum sampleLight;
  	if(scene->lights[i]->is_delta_light()){
  		singleSample = scene->lights[i]->sample_L(hit_p, &wi, &distToLight, &pdf);
  		w_in = w2o * wi;
  		//cout << "w_in.z " << w_in.z << endl;
  		if(w_in.z < 0){
  			continue;
  		}
  		else{
  			//cout << "else " << endl;
  			Ray shadowRay = Ray(EPS_D * w_in + hit_p, w_in);
  			shadowRay.max_t = distToLight;			//this sample radiance
  			Intersection thisisect;					//this sample intersection
  			sampleLight = (0,0,0);
  			//cout << "intersect " << endl;
  			if(bvh->intersect(shadowRay, &thisisect)){
  				//Spectrum emission = thisisect.bsdf->get_emission();					//emission is Li
  				Spectrum f = isect.bsdf->f(w_out, wi);								// f is ambient light

  				sampleLight = (singleSample * f /** emission*/ * w_in.z ) / pdf;
  			}
  			else{
  				Spectrum f = isect.bsdf->f(w_out, wi);
	  			sampleLight = f / pdf;
  			}
  			//cout << "before " << endl;
  			L_out += (sampleLight);
  	...
  	//if not delta
  		...
  		for(int j=0; j < ns_area_light; j++){
	  		...
	  		if(w_in.z < 0){
	  			...
	  			sampleLight = (0,0,0);						//this sample radiance
	  			Intersection thisisect;					//this sample intersection
	  			...
	  			if(bvh->intersect(shadowRay, &thisisect)){
	  				...
	  				Spectrum f = isect.bsdf->f(w_out, wi);
	  				sampleLight = (singleSample * f) / pdf;
	  			}
	  			areaSample += (sampleLight);
	  		}
  		}
  		L_out += (areaSample / ns_area_light);
  		//L_out /= numOf_samples;
  	}
  }
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  float pdf = 0.0;
  double multConst = 11;
  double addFactor = 0.05;

  Spectrum sampleHere = isect.bsdf->sample_f(w_out, &wi, &pdf);
  Vector3D w_in = o2w * wi;
  //w_in.normalize();
  double radiance = (double) sampleHere.illum();
  //cout << "rad " << radiance << endl;
  radiance = radiance * multConst + addFactor;
  //if(radiance > 255)	radiance = 255; 

  //double russRoulProb = random_uniform();
  //double notTermProb = 1 - russRoulProb; 
  //double tempRad = radiance / notTermProb;

  Spectrum incomeRadiance;
  bool termiation = coin_flip(radiance);
  if(!termiation){
  	return (Spectrum());
  }
  //else{
  	Ray bounceRay = Ray(hit_p + (EPS_D), w_in, (int)r.depth-1);
  	//bounceRay.depth =  r.depth - 1; 

  	incomeRadiance = trace_ray(bounceRay, isect.bsdf->is_delta());
  //}
  
  float cos = max(0.0, wi.z);
  Spectrum outgoingRadiance = (incomeRadiance * cos * sampleHere) / (pdf * radiance);
