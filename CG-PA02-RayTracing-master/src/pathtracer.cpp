...
//https://github.com/cmu462/Scotty3D/wiki/(Task-1)-Generating-Camera-Rays
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1) {
    Ray r = camera->generate_ray((double) (origin.x + 0.5) / (double) sampleBuffer.w , (double) (origin.y + 0.5) / (double) sampleBuffer.h); // replace 1.0 with your values based on x,y,sampleBuffer.w,sampleBuffer.h
    ...
    //float image_ar = sampleBuffer.w/sampleBuffer.h;
    for(int i=0; i < num_samples; i++){
      Vector2D offset = gridSampler->get_sample();      
      Ray r = camera->generate_ray((origin.x + offset.x) / (double) sampleBuffer.w, (origin.y + offset.y) / (double) sampleBuffer.h);
      //Ray r = camera->generate_ray((origin.x + 0.5) / (double) sampleBuffer.w , (origin.y + 0.5) / (double) sampleBuffer.h);
      r.depth = max_ray_depth;
      avg += trace_ray(r, true);
    }
    avg = avg / num_samples;
    output = avg;
    sampleCountBuffer[index] = num_samples;
