...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //Vector3D pixelCoor = Vector3D(x, y, 0); 
  Vector3D o = pos;
  ...
  //Vector3D cameraSize = bottom_left + (top_right - bottom_left);

  double us = bottom_left.x + ((top_right.x - bottom_left.x)*x);
  double vs = bottom_left.y + ((top_right.y - bottom_left.y)*y);

  Vector3D dir = /*bottom_left + */Vector3D(us, vs, -1);
  //double ws_us = c2w*us;
  //double ws_vs = c2w*vs;
  dir = c2w*dir;
  dir.normalize();
